...
export const ConfirmDialog: React.FC<ConfirmDialogProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div
        ref={dialogRef}
        className="relative bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-sm p-6 animate-in fade-in zoom-in-95 duration-150"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-start gap-4 mb-6">
          ...
          <div>
            <h3 id="confirm-dialog-title" className="font-semibold text-slate-900 mb-1">{title}</h3>
            <p id="confirm-dialog-message" className="text-sm text-slate-500">{message}</p>
          </div>
        </div>
